...
class WXDLLIMPEXP_SMTP wxSMTP : public wxCmdlineProtocol
{
   public:
      ...
      class WXDLLIMPEXP_SMTP Listener
      {
         public:
             ...
             * \param status The ::SendingStatus_t of the message
             ...
             */
            virtual void OnMessageStatus(const mimetic::MessageId&                  WXUNUSED(message_id),
       ...
       */
      void SendMessage(const mimetic::MimeEntity& message, bool shall_start_sending_process = true);
   ...
   private:
       ...
       *
       * It invokes the Listener::OnMessageStatus.
       *
       * If the user selects continue, the state ::wxSMTP::SendMailFromState is entered. Else
       * ::wxSMTP::g_quitState is entered.
       */
      void CancelMailSendingProcess(Listener::SendingStatus_t status, bool shall_quit, bool shall_send_quit, Listener::DisconnectionStatus_t disconnection_status);
       ...
       * #retry_messages_list.
       */
      std::list<mimetic::MimeEntity> messages_to_send;
       ...
       * by the RCPT state.
       */
      mimetic::Mailbox current_recipient;
